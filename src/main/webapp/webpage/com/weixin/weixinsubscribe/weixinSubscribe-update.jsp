	...
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		...
		<table style="width: 600px;" cellpadding="0" cellspacing="1"
			...
			<tr>
				...
				<td class="value">
					<%-- <input id="msgtype" name="msgtype"
					type="text" style="width: 150px" class="inputxt"
					value='${weixinSubscribePage.msgtype}'> --%> <select name="msgtype"
					id="msgtype" datatype="*">
						<option value="text"
							<c:if test="${weixinSubscribePage.msgtype=='text'}">selected="selected"</c:if>>文本消息</option>
						<option value="news"
							<c:if test="${weixinSubscribePage.msgtype=='news'}">selected="selected"</c:if>>图文消息</option>
					...
					id="textContent" datatype="*">
						<c:forEach items="${textList}" var="textVal">
							<option value="${textVal.id}" name="textMsg"
								<c:if test="${weixinSubscribePage.templateid==textVal.id}">selected="selected"</c:if>>${textVal.templatename}</option>
						</c:forEach>
				</select></td>
					...
					name="templateid" id="newsContent" disabled="disabled" datatype="*">
						<c:forEach items="${newsList}" var="newsVal">
							<option value="${newsVal.id}"
								<c:if test="${weixinSubscribePage.templateid==newsVal.id}">selected="selected"</c:if>>${newsVal.templatename}</option>
						</c:forEach>
				</select> <span class="Validform_checktip"></span> <label
