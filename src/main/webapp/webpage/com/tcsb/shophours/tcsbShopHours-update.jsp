...
<body>
	<t:formvalid formid="formobj" dialog="true" usePlugin="password"
		layout="table" action="tcsbShopHoursController.do?doUpdate">
		...
		<table style="width: 600px;" cellpadding="0" cellspacing="1"
				...
				<td class="value"><select id="shopHoursPeriodId"
					name="shopHoursPeriodId" datatype="*">
						...
						<c:forEach items="${tcsbShopHoursPeriodEntitys}"
							...
							<option value="${tcsbShopHoursPeriodEntity.id}"
								<c:if test="${tcsbShopHoursPeriodEntity.id==tcsbShopHoursPage.shopHoursPeriodId }">selected ="selected"</c:if>>${tcsbShopHoursPeriodEntity.name}</option>

						</c:forEach>
				</select> <span class="Validform_checktip"></span> <label
					class="Validform_label" style="display: none;">营业时间段</label></td>
			<tr>
			<tr>
				...
				<td class="value"><input id="beginTime1" type="text"
					...
					value="${fn:substring(tcsbShopHoursPage.beginTime,11,16) }"
					datatype="*"> <input id="beginTime" name="beginTime"
					type="text" value="${tcsbShopHoursPage.beginTime }"
			...
			<tr>
				...
				<td class="value"><input id="endTime1" type="text"
					style="width: 150px" onchange="dateInit1(this.value)"
					value="${fn:substring(tcsbShopHoursPage.endTime,11,16)}"
					class="Wdate" onClick="WdatePicker({dateFmt:'HH:mm'})" datatype="*">
					<input id="endTime" name="endTime" type="text"
					value="${tcsbShopHoursPage.endTime }" style="display: none" /> <span
