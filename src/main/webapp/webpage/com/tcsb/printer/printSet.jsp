...
<script type="text/javascript">
	$(function() {
		$('#functionid').tree(
				{
					checkbox : true,
					url : 'tcsbPrinterController.do?setAuthority2&printerId=${printerId}',
					...
					onClick : function(node) {
						...
						if (isRoot == '') {
							...
							/* $('#operationListpanel').panel(
									"refresh",
									"tcsbPrinterController.do?operationListForFunction&functionId="+ node.id + "&printId=" + printId); */
						} else {
						}
					}
				});
	...
	function mysubmit() {
		var printerId = $("#rid").val();
		var s = GetNode();
		doSubmit("tcsbPrinterController.do?updateAuthority2&printfunctions=" + s
				+ "&printerId=" + printerId);
	}
	function GetNode() {
	...
	function selecrAll() {
		...
		for ( var i = 0; i < node.length; i++) {
			...
			$('#functionid').tree("check", node[i].target);
		}
	}
	function reset() {
...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding: 1px;">
		<div class="easyui-panel" style="padding: 1px;" fit="true"
			...
			<input type="hidden" name="printerId" value="${printerId}" id="rid">
