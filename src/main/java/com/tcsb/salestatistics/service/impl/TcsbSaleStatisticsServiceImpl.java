package com.tcsb.salestatistics.service.impl;
...
@Service("tcsbSaleStatisticsService")
@Transactional
public class TcsbSaleStatisticsServiceImpl extends CommonServiceImpl implements TcsbSaleStatisticsServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(TcsbSaleStatisticsEntity t) throws Exception{
 	}
 	/**
	 * 更新操作增强业务
	 * @param t
	 * @return
	 */
	private void doUpdateBus(TcsbSaleStatisticsEntity t) throws Exception{
 	}
 	/**
	 * 删除操作增强业务
	 * @param id
	 * @return
	 */
	private void doDelBus(TcsbSaleStatisticsEntity t) throws Exception{
 	}
 	
 	private Map<String,Object> populationMap(TcsbSaleStatisticsEntity t){
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("id", t.getId());
		map.put("food_id", t.getFoodId());
		map.put("count", t.getCount());
		map.put("food_name", t.getFoodName());
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,TcsbSaleStatisticsEntity t){
 		sql  = sql.replace("#{id}",String.valueOf(t.getId()));
 		sql  = sql.replace("#{food_id}",String.valueOf(t.getFoodId()));
 		sql  = sql.replace("#{count}",String.valueOf(t.getCount()));
 		sql  = sql.replace("#{food_name}",String.valueOf(t.getFoodName()));
