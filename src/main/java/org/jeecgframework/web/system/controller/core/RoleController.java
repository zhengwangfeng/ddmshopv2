...
public class RoleController extends BaseController {
	...
	public AjaxJson saveRole(TSRole role, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(role.getId())) {
	        ...
	        if (!checkAdmin()) {
	        	TcsbShopEntity tcsbShopEntity = systemService.findUniqueByProperty(TcsbShopEntity.class, "userId", tsUser.getId());
	        	role.setShopId(tcsbShopEntity.getId());
			}
	        /*更新字断shopId--------end-------过滤用户权限*/
	...
	public List<ComboTree> setAuthority(TSRole role,
		...
		cq.notEq("functionLevel", Short.parseShort("-1"));
		 //系统更新update by jimmy 如果是商家只读出商家权限,商家只有一种角色
        /*更新字断shopId--------begin-------过滤用户权限*/
		List<TSFunction> functionList = new ArrayList<>();
        TSUser tsUser = getCurrentUser();
        if (!checkAdmin()) {
        	//获取角色id商家只有一种角色
        	TSRoleUser tsRoleUser = systemService.findUniqueByProperty(TSRoleUser.class, "TSUser.id", tsUser.getId());
        	//根据角色id获取商家的菜单权限
        	List<TSRoleFunction> tsRoleFunctions = systemService.findHql("from TSRoleFunction where TSRole.id = ?", tsRoleUser.getTSRole().getId());
        	List<String> args = new ArrayList<>();
        	if (!tsRoleFunctions.isEmpty()) {
				for (int i = 0; i < tsRoleFunctions.size(); i++) {
					args.add(tsRoleFunctions.get(i).getTSFunction().getId());
				}
			}
        	CriteriaQuery cqt = new CriteriaQuery(TSFunction.class);
        	if (comboTree.getId() != null) {
    			cqt.eq("TSFunction.id", comboTree.getId());
    		}
    		if (comboTree.getId() == null) {
    			cqt.isNull("TSFunction");
    		}
    		cqt.notEq("functionLevel", Short.parseShort("-1"));
        	cqt.add(Restrictions.in("id", args.toArray()));
        	cqt.add();
        	functionList =  systemService.getListByCriteriaQuery(
    				cqt, false);
		}else {
			cq.add();
			functionList = systemService.getListByCriteriaQuery(
		...
		//更新自定义方法过滤字权限update by jimmy----begin----//
		if (!checkAdmin()) {
    		for (TSFunction tsFunction : functionList) {
    			comboTrees.add(comboTree(tsFunction, loginActionlist,tsUser, true));
    		}
    		functionList.clear();
		}
		else {
		...
		//更新自定义方法过滤字权限update by jimmy----end----//
		//ComboTreeModel comboTreeModel = new ComboTreeModel("id","functionName", "TSFunctions");
		//comboTrees = systemService.ComboTree(functionList, comboTreeModel,loginActionlist, true);
	...
	private ComboTree comboTree(TSFunction tsFunction,
			List<TSFunction> loginActionlist,TSUser tsUser, boolean b) {
		//获取角色id商家只有一种角色
    	TSRoleUser tsRoleUser = systemService.findUniqueByProperty(TSRoleUser.class, "TSUser.id", tsUser.getId());
    	//根据角色id获取商家的菜单权限
    	List<TSRoleFunction> tsRoleFunctions = systemService.findHql("from TSRoleFunction where TSRole.id = ?", tsRoleUser.getTSRole().getId());
		...
		tree.setId(tsFunction.getId());
		tree.setText(tsFunction.getFunctionName());
		if (loginActionlist == null) {
		} else {
			if (loginActionlist.size() > 0) {
				for (TSFunction inobj : loginActionlist) {
					String inId = inobj.getId();
                    if (inId.equals(tsFunction.getId())) {
		...
		//只选择商家的菜单权限
		List<TSFunction> curChildListAll = tsFunction.getTSFunctions();
		List<TSFunction> curChildList = new ArrayList<>();
		for (TSFunction tFunction : curChildListAll) {
			for (int i = 0; i < args.size(); i++) {
				if (tFunction.getId().equals(args.get(i))) {
					curChildList.add(tFunction);
					break;
				}
			}
		}
		...
		if (curChildList != null && curChildList.size() > 0) {
            ...
            if (b) { // 递归查询子节点
                ...
                List<TSFunction> nextChildList = new ArrayList(curChildList);
                for (TSFunction childObj : nextChildList) {
                    ComboTree t = comboTree(childObj, loginActionlist, tsUser, b);
                    children.add(t);
                }
